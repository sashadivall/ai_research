...
def score_theory(data: pd.DataFrame) -> None:
    ...
    Return a string dictionary with a key `values` containing an array of floats, {"values": [score1, score2, ...]}.
    **DO NOT FORMAT THE STRING AS JSON**. Return the dictionary as a string
    ...
    results = my_chat.score_multiple_courses(intro_description)
...
def score_ethics_course(data: pd.DataFrame) -> None:
    instructions = """
    You are an expert in AI ethics and curriculum evaluation. 
    ...
    **DO NOT FORMAT THE STRING AS JSON**. Return the dictionary as a string
    ...
    data["Ethics Course Score"] = results
